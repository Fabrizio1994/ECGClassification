...
class Preprocessing():
    ...
    def segment(self, classes, names, aami=True, one_hot=True):
        X = list()
        Y = list()
        for name in names:
            X_signal = list()
            ...
            for i, p in enumerate(peaks[1:-1], start=1):
                left_end = peaks[i ]-300
                right_end = peaks[i]+300
                beat = record[left_end:right_end]
                label = np.argmax(labels[i])
                X_signal.append(beat)
                Y.append(label)
            X.extend(X_signal)
...
if __name__ == '__main__':
    pre = Preprocessing()
    X, Y = (pre.segment(['N', 'S', 'V', 'F'], names = list(
            {'100', '101', '102', '103', '104', '105', '106', '107', '108', '109', '111', '112', '113', '114', '115',
             '116', '117', '118', '119', '121', '122', '123', '124', '200', '201', '202', '203', '205', '207',
             '208',
             '209', '210', '212', '213', '214', '215', '217', '219', '220', '221', '222', '223', '228', '230', '231',
             '232', '233', '234'} - {'102', '104', '107', '217', '207'})))